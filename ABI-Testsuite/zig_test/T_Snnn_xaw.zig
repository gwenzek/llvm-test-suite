...
test "C_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_C(c.ret_C_C()));
}
...
test "C_C_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_C_C(c.ret_C_C_C()));
}
...
test "C_C_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_C_D(c.ret_C_C_D()));
}
...
test "C_C_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_C_F(c.ret_C_C_F()));
}
...
test "C_C_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_C_I(c.ret_C_C_I()));
}
...
test "C_C_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_C_Ip(c.ret_C_C_Ip()));
}
...
test "C_C_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_C_L(c.ret_C_C_L()));
}
...
test "C_C_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_C_S(c.ret_C_C_S()));
}
...
test "C_C_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_C_Uc(c.ret_C_C_Uc()));
}
...
test "C_C_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_C_Ui(c.ret_C_C_Ui()));
}
...
test "C_C_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_C_Ul(c.ret_C_C_Ul()));
}
...
test "C_C_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_C_Us(c.ret_C_C_Us()));
}
...
test "C_C_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_C_Vp(c.ret_C_C_Vp()));
}
...
test "C_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_D(c.ret_C_D()));
}
...
test "C_D_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_D_C(c.ret_C_D_C()));
}
...
test "C_D_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_D_D(c.ret_C_D_D()));
}
...
test "C_D_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_D_F(c.ret_C_D_F()));
}
...
test "C_D_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_D_I(c.ret_C_D_I()));
}
...
test "C_D_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_D_Ip(c.ret_C_D_Ip()));
}
...
test "C_D_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_D_L(c.ret_C_D_L()));
}
...
test "C_D_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_D_S(c.ret_C_D_S()));
}
...
test "C_D_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_D_Uc(c.ret_C_D_Uc()));
}
...
test "C_D_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_D_Ui(c.ret_C_D_Ui()));
}
...
test "C_D_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_D_Ul(c.ret_C_D_Ul()));
}
...
test "C_D_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_D_Us(c.ret_C_D_Us()));
}
...
test "C_D_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_D_Vp(c.ret_C_D_Vp()));
}
...
test "C_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_F(c.ret_C_F()));
}
...
test "C_F_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_F_C(c.ret_C_F_C()));
}
...
test "C_F_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_F_D(c.ret_C_F_D()));
}
...
test "C_F_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_F_F(c.ret_C_F_F()));
}
...
test "C_F_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_F_I(c.ret_C_F_I()));
}
...
test "C_F_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_F_Ip(c.ret_C_F_Ip()));
}
...
test "C_F_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_F_L(c.ret_C_F_L()));
}
...
test "C_F_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_F_S(c.ret_C_F_S()));
}
...
test "C_F_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_F_Uc(c.ret_C_F_Uc()));
}
...
test "C_F_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_F_Ui(c.ret_C_F_Ui()));
}
...
test "C_F_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_F_Ul(c.ret_C_F_Ul()));
}
...
test "C_F_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_F_Us(c.ret_C_F_Us()));
}
...
test "C_F_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_F_Vp(c.ret_C_F_Vp()));
}
...
test "C_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_I(c.ret_C_I()));
}
...
test "C_I_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_I_C(c.ret_C_I_C()));
}
...
test "C_I_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_I_D(c.ret_C_I_D()));
}
...
test "C_I_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_I_F(c.ret_C_I_F()));
}
...
test "C_I_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_I_I(c.ret_C_I_I()));
}
...
test "C_I_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_I_Ip(c.ret_C_I_Ip()));
}
...
test "C_I_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_I_L(c.ret_C_I_L()));
}
...
test "C_I_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_I_S(c.ret_C_I_S()));
}
...
test "C_I_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_I_Uc(c.ret_C_I_Uc()));
}
...
test "C_I_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_I_Ui(c.ret_C_I_Ui()));
}
...
test "C_I_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_I_Ul(c.ret_C_I_Ul()));
}
...
test "C_I_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_I_Us(c.ret_C_I_Us()));
}
...
test "C_I_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_I_Vp(c.ret_C_I_Vp()));
}
...
test "C_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Ip(c.ret_C_Ip()));
}
...
test "C_Ip_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Ip_C(c.ret_C_Ip_C()));
}
...
test "C_Ip_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Ip_D(c.ret_C_Ip_D()));
}
...
test "C_Ip_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Ip_F(c.ret_C_Ip_F()));
}
...
test "C_Ip_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Ip_I(c.ret_C_Ip_I()));
}
...
test "C_Ip_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Ip_Ip(c.ret_C_Ip_Ip()));
}
...
test "C_Ip_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Ip_L(c.ret_C_Ip_L()));
}
...
test "C_Ip_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Ip_S(c.ret_C_Ip_S()));
}
...
test "C_Ip_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Ip_Uc(c.ret_C_Ip_Uc()));
}
...
test "C_Ip_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Ip_Ui(c.ret_C_Ip_Ui()));
}
...
test "C_Ip_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Ip_Ul(c.ret_C_Ip_Ul()));
}
...
test "C_Ip_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Ip_Us(c.ret_C_Ip_Us()));
}
...
test "C_Ip_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Ip_Vp(c.ret_C_Ip_Vp()));
}
...
test "C_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_L(c.ret_C_L()));
}
...
test "C_L_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_L_C(c.ret_C_L_C()));
}
...
test "C_L_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_L_D(c.ret_C_L_D()));
}
...
test "C_L_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_L_F(c.ret_C_L_F()));
}
...
test "C_L_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_L_I(c.ret_C_L_I()));
}
...
test "C_L_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_L_Ip(c.ret_C_L_Ip()));
}
...
test "C_L_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_L_L(c.ret_C_L_L()));
}
...
test "C_L_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_L_S(c.ret_C_L_S()));
}
...
test "C_L_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_L_Uc(c.ret_C_L_Uc()));
}
...
test "C_L_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_L_Ui(c.ret_C_L_Ui()));
}
...
test "C_L_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_L_Ul(c.ret_C_L_Ul()));
}
...
test "C_L_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_L_Us(c.ret_C_L_Us()));
}
...
test "C_L_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_L_Vp(c.ret_C_L_Vp()));
}
...
test "C_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_S(c.ret_C_S()));
}
...
test "C_S_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_S_C(c.ret_C_S_C()));
}
...
test "C_S_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_S_D(c.ret_C_S_D()));
}
...
test "C_S_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_S_F(c.ret_C_S_F()));
}
...
test "C_S_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_S_I(c.ret_C_S_I()));
}
...
test "C_S_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_S_Ip(c.ret_C_S_Ip()));
}
...
test "C_S_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_S_L(c.ret_C_S_L()));
}
...
test "C_S_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_S_S(c.ret_C_S_S()));
}
...
test "C_S_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_S_Uc(c.ret_C_S_Uc()));
}
...
test "C_S_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_S_Ui(c.ret_C_S_Ui()));
}
...
test "C_S_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_S_Ul(c.ret_C_S_Ul()));
}
...
test "C_S_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_S_Us(c.ret_C_S_Us()));
}
...
test "C_S_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_S_Vp(c.ret_C_S_Vp()));
}
...
test "C_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Uc(c.ret_C_Uc()));
}
...
test "C_Uc_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Uc_C(c.ret_C_Uc_C()));
}
...
test "C_Uc_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Uc_D(c.ret_C_Uc_D()));
}
...
test "C_Uc_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Uc_F(c.ret_C_Uc_F()));
}
...
test "C_Uc_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Uc_I(c.ret_C_Uc_I()));
}
...
test "C_Uc_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Uc_Ip(c.ret_C_Uc_Ip()));
}
...
test "C_Uc_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Uc_L(c.ret_C_Uc_L()));
}
...
test "C_Uc_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Uc_S(c.ret_C_Uc_S()));
}
...
test "C_Uc_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Uc_Uc(c.ret_C_Uc_Uc()));
}
...
test "C_Uc_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Uc_Ui(c.ret_C_Uc_Ui()));
}
...
test "C_Uc_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Uc_Ul(c.ret_C_Uc_Ul()));
}
...
test "C_Uc_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Uc_Us(c.ret_C_Uc_Us()));
}
...
test "C_Uc_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Uc_Vp(c.ret_C_Uc_Vp()));
}
...
test "C_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Ui(c.ret_C_Ui()));
}
...
test "C_Ui_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Ui_C(c.ret_C_Ui_C()));
}
...
test "C_Ui_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Ui_D(c.ret_C_Ui_D()));
}
...
test "C_Ui_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Ui_F(c.ret_C_Ui_F()));
}
...
test "C_Ui_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Ui_I(c.ret_C_Ui_I()));
}
...
test "C_Ui_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Ui_Ip(c.ret_C_Ui_Ip()));
}
...
test "C_Ui_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Ui_L(c.ret_C_Ui_L()));
}
...
test "C_Ui_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Ui_S(c.ret_C_Ui_S()));
}
...
test "C_Ui_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Ui_Uc(c.ret_C_Ui_Uc()));
}
...
test "C_Ui_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Ui_Ui(c.ret_C_Ui_Ui()));
}
...
test "C_Ui_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Ui_Ul(c.ret_C_Ui_Ul()));
}
...
test "C_Ui_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Ui_Us(c.ret_C_Ui_Us()));
}
...
test "C_Ui_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Ui_Vp(c.ret_C_Ui_Vp()));
}
...
test "C_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Ul(c.ret_C_Ul()));
}
...
test "C_Ul_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Ul_C(c.ret_C_Ul_C()));
}
...
test "C_Ul_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Ul_D(c.ret_C_Ul_D()));
}
...
test "C_Ul_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Ul_F(c.ret_C_Ul_F()));
}
...
test "C_Ul_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Ul_I(c.ret_C_Ul_I()));
}
...
test "C_Ul_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Ul_Ip(c.ret_C_Ul_Ip()));
}
...
test "C_Ul_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Ul_L(c.ret_C_Ul_L()));
}
...
test "C_Ul_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Ul_S(c.ret_C_Ul_S()));
}
...
test "C_Ul_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Ul_Uc(c.ret_C_Ul_Uc()));
}
...
test "C_Ul_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Ul_Ui(c.ret_C_Ul_Ui()));
}
...
test "C_Ul_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Ul_Ul(c.ret_C_Ul_Ul()));
}
...
test "C_Ul_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Ul_Us(c.ret_C_Ul_Us()));
}
...
test "C_Ul_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Ul_Vp(c.ret_C_Ul_Vp()));
}
...
test "C_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Us(c.ret_C_Us()));
}
...
test "C_Us_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Us_C(c.ret_C_Us_C()));
}
...
test "C_Us_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Us_D(c.ret_C_Us_D()));
}
...
test "C_Us_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Us_F(c.ret_C_Us_F()));
}
...
test "C_Us_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Us_I(c.ret_C_Us_I()));
}
...
test "C_Us_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Us_Ip(c.ret_C_Us_Ip()));
}
...
test "C_Us_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Us_L(c.ret_C_Us_L()));
}
...
test "C_Us_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Us_S(c.ret_C_Us_S()));
}
...
test "C_Us_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Us_Uc(c.ret_C_Us_Uc()));
}
...
test "C_Us_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Us_Ui(c.ret_C_Us_Ui()));
}
...
test "C_Us_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Us_Ul(c.ret_C_Us_Ul()));
}
...
test "C_Us_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Us_Us(c.ret_C_Us_Us()));
}
...
test "C_Us_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Us_Vp(c.ret_C_Us_Vp()));
}
...
test "C_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Vp(c.ret_C_Vp()));
}
...
test "C_Vp_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Vp_C(c.ret_C_Vp_C()));
}
...
test "C_Vp_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Vp_D(c.ret_C_Vp_D()));
}
...
test "C_Vp_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Vp_F(c.ret_C_Vp_F()));
}
...
test "C_Vp_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Vp_I(c.ret_C_Vp_I()));
}
...
test "C_Vp_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Vp_Ip(c.ret_C_Vp_Ip()));
}
...
test "C_Vp_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Vp_L(c.ret_C_Vp_L()));
}
...
test "C_Vp_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Vp_S(c.ret_C_Vp_S()));
}
...
test "C_Vp_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Vp_Uc(c.ret_C_Vp_Uc()));
}
...
test "C_Vp_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Vp_Ui(c.ret_C_Vp_Ui()));
}
...
test "C_Vp_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Vp_Ul(c.ret_C_Vp_Ul()));
}
...
test "C_Vp_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Vp_Us(c.ret_C_Vp_Us()));
}
...
test "C_Vp_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_C_Vp_Vp(c.ret_C_Vp_Vp()));
}
...
test "D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D(c.ret_D()));
}
...
test "D_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_C(c.ret_D_C()));
}
...
test "D_C_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_C_C(c.ret_D_C_C()));
}
...
test "D_C_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_C_D(c.ret_D_C_D()));
}
...
test "D_C_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_C_F(c.ret_D_C_F()));
}
...
test "D_C_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_C_I(c.ret_D_C_I()));
}
...
test "D_C_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_C_Ip(c.ret_D_C_Ip()));
}
...
test "D_C_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_C_L(c.ret_D_C_L()));
}
...
test "D_C_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_C_S(c.ret_D_C_S()));
}
...
test "D_C_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_C_Uc(c.ret_D_C_Uc()));
}
...
test "D_C_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_C_Ui(c.ret_D_C_Ui()));
}
...
test "D_C_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_C_Ul(c.ret_D_C_Ul()));
}
...
test "D_C_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_C_Us(c.ret_D_C_Us()));
}
...
test "D_C_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_C_Vp(c.ret_D_C_Vp()));
}
...
test "D_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_D(c.ret_D_D()));
}
...
test "D_D_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_D_C(c.ret_D_D_C()));
}
...
test "D_D_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_D_D(c.ret_D_D_D()));
}
...
test "D_D_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_D_F(c.ret_D_D_F()));
}
...
test "D_D_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_D_I(c.ret_D_D_I()));
}
...
test "D_D_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_D_Ip(c.ret_D_D_Ip()));
}
...
test "D_D_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_D_L(c.ret_D_D_L()));
}
...
test "D_D_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_D_S(c.ret_D_D_S()));
}
...
test "D_D_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_D_Uc(c.ret_D_D_Uc()));
}
...
test "D_D_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_D_Ui(c.ret_D_D_Ui()));
}
...
test "D_D_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_D_Ul(c.ret_D_D_Ul()));
}
...
test "D_D_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_D_Us(c.ret_D_D_Us()));
}
...
test "D_D_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_D_Vp(c.ret_D_D_Vp()));
}
...
test "D_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_F(c.ret_D_F()));
}
...
test "D_F_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_F_C(c.ret_D_F_C()));
}
...
test "D_F_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_F_D(c.ret_D_F_D()));
}
...
test "D_F_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_F_F(c.ret_D_F_F()));
}
...
test "D_F_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_F_I(c.ret_D_F_I()));
}
...
test "D_F_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_F_Ip(c.ret_D_F_Ip()));
}
...
test "D_F_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_F_L(c.ret_D_F_L()));
}
...
test "D_F_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_F_S(c.ret_D_F_S()));
}
...
test "D_F_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_F_Uc(c.ret_D_F_Uc()));
}
...
test "D_F_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_F_Ui(c.ret_D_F_Ui()));
}
...
test "D_F_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_F_Ul(c.ret_D_F_Ul()));
}
...
test "D_F_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_F_Us(c.ret_D_F_Us()));
}
...
test "D_F_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_F_Vp(c.ret_D_F_Vp()));
}
...
test "D_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_I(c.ret_D_I()));
}
...
test "D_I_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_I_C(c.ret_D_I_C()));
}
...
test "D_I_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_I_D(c.ret_D_I_D()));
}
...
test "D_I_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_I_F(c.ret_D_I_F()));
}
...
test "D_I_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_I_I(c.ret_D_I_I()));
}
...
test "D_I_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_I_Ip(c.ret_D_I_Ip()));
}
...
test "D_I_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_I_L(c.ret_D_I_L()));
}
...
test "D_I_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_I_S(c.ret_D_I_S()));
}
...
test "D_I_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_I_Uc(c.ret_D_I_Uc()));
}
...
test "D_I_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_I_Ui(c.ret_D_I_Ui()));
}
...
test "D_I_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_I_Ul(c.ret_D_I_Ul()));
}
...
test "D_I_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_I_Us(c.ret_D_I_Us()));
}
...
test "D_I_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_I_Vp(c.ret_D_I_Vp()));
}
...
test "D_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Ip(c.ret_D_Ip()));
}
...
test "D_Ip_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Ip_C(c.ret_D_Ip_C()));
}
...
test "D_Ip_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Ip_D(c.ret_D_Ip_D()));
}
...
test "D_Ip_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Ip_F(c.ret_D_Ip_F()));
}
...
test "D_Ip_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Ip_I(c.ret_D_Ip_I()));
}
...
test "D_Ip_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Ip_Ip(c.ret_D_Ip_Ip()));
}
...
test "D_Ip_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Ip_L(c.ret_D_Ip_L()));
}
...
test "D_Ip_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Ip_S(c.ret_D_Ip_S()));
}
...
test "D_Ip_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Ip_Uc(c.ret_D_Ip_Uc()));
}
...
test "D_Ip_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Ip_Ui(c.ret_D_Ip_Ui()));
}
...
test "D_Ip_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Ip_Ul(c.ret_D_Ip_Ul()));
}
...
test "D_Ip_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Ip_Us(c.ret_D_Ip_Us()));
}
...
test "D_Ip_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Ip_Vp(c.ret_D_Ip_Vp()));
}
...
test "D_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_L(c.ret_D_L()));
}
...
test "D_L_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_L_C(c.ret_D_L_C()));
}
...
test "D_L_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_L_D(c.ret_D_L_D()));
}
...
test "D_L_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_L_F(c.ret_D_L_F()));
}
...
test "D_L_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_L_I(c.ret_D_L_I()));
}
...
test "D_L_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_L_Ip(c.ret_D_L_Ip()));
}
...
test "D_L_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_L_L(c.ret_D_L_L()));
}
...
test "D_L_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_L_S(c.ret_D_L_S()));
}
...
test "D_L_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_L_Uc(c.ret_D_L_Uc()));
}
...
test "D_L_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_L_Ui(c.ret_D_L_Ui()));
}
...
test "D_L_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_L_Ul(c.ret_D_L_Ul()));
}
...
test "D_L_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_L_Us(c.ret_D_L_Us()));
}
...
test "D_L_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_L_Vp(c.ret_D_L_Vp()));
}
...
test "D_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_S(c.ret_D_S()));
}
...
test "D_S_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_S_C(c.ret_D_S_C()));
}
...
test "D_S_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_S_D(c.ret_D_S_D()));
}
...
test "D_S_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_S_F(c.ret_D_S_F()));
}
...
test "D_S_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_S_I(c.ret_D_S_I()));
}
...
test "D_S_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_S_Ip(c.ret_D_S_Ip()));
}
...
test "D_S_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_S_L(c.ret_D_S_L()));
}
...
test "D_S_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_S_S(c.ret_D_S_S()));
}
...
test "D_S_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_S_Uc(c.ret_D_S_Uc()));
}
...
test "D_S_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_S_Ui(c.ret_D_S_Ui()));
}
...
test "D_S_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_S_Ul(c.ret_D_S_Ul()));
}
...
test "D_S_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_S_Us(c.ret_D_S_Us()));
}
...
test "D_S_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_S_Vp(c.ret_D_S_Vp()));
}
...
test "D_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Uc(c.ret_D_Uc()));
}
...
test "D_Uc_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Uc_C(c.ret_D_Uc_C()));
}
...
test "D_Uc_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Uc_D(c.ret_D_Uc_D()));
}
...
test "D_Uc_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Uc_F(c.ret_D_Uc_F()));
}
...
test "D_Uc_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Uc_I(c.ret_D_Uc_I()));
}
...
test "D_Uc_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Uc_Ip(c.ret_D_Uc_Ip()));
}
...
test "D_Uc_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Uc_L(c.ret_D_Uc_L()));
}
...
test "D_Uc_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Uc_S(c.ret_D_Uc_S()));
}
...
test "D_Uc_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Uc_Uc(c.ret_D_Uc_Uc()));
}
...
test "D_Uc_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Uc_Ui(c.ret_D_Uc_Ui()));
}
...
test "D_Uc_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Uc_Ul(c.ret_D_Uc_Ul()));
}
...
test "D_Uc_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Uc_Us(c.ret_D_Uc_Us()));
}
...
test "D_Uc_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Uc_Vp(c.ret_D_Uc_Vp()));
}
...
test "D_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Ui(c.ret_D_Ui()));
}
...
test "D_Ui_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Ui_C(c.ret_D_Ui_C()));
}
...
test "D_Ui_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Ui_D(c.ret_D_Ui_D()));
}
...
test "D_Ui_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Ui_F(c.ret_D_Ui_F()));
}
...
test "D_Ui_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Ui_I(c.ret_D_Ui_I()));
}
...
test "D_Ui_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Ui_Ip(c.ret_D_Ui_Ip()));
}
...
test "D_Ui_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Ui_L(c.ret_D_Ui_L()));
}
...
test "D_Ui_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Ui_S(c.ret_D_Ui_S()));
}
...
test "D_Ui_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Ui_Uc(c.ret_D_Ui_Uc()));
}
...
test "D_Ui_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Ui_Ui(c.ret_D_Ui_Ui()));
}
...
test "D_Ui_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Ui_Ul(c.ret_D_Ui_Ul()));
}
...
test "D_Ui_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Ui_Us(c.ret_D_Ui_Us()));
}
...
test "D_Ui_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Ui_Vp(c.ret_D_Ui_Vp()));
}
...
test "D_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Ul(c.ret_D_Ul()));
}
...
test "D_Ul_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Ul_C(c.ret_D_Ul_C()));
}
...
test "D_Ul_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Ul_D(c.ret_D_Ul_D()));
}
...
test "D_Ul_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Ul_F(c.ret_D_Ul_F()));
}
...
test "D_Ul_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Ul_I(c.ret_D_Ul_I()));
}
...
test "D_Ul_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Ul_Ip(c.ret_D_Ul_Ip()));
}
...
test "D_Ul_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Ul_L(c.ret_D_Ul_L()));
}
...
test "D_Ul_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Ul_S(c.ret_D_Ul_S()));
}
...
test "D_Ul_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Ul_Uc(c.ret_D_Ul_Uc()));
}
...
test "D_Ul_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Ul_Ui(c.ret_D_Ul_Ui()));
}
...
test "D_Ul_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Ul_Ul(c.ret_D_Ul_Ul()));
}
...
test "D_Ul_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Ul_Us(c.ret_D_Ul_Us()));
}
...
test "D_Ul_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Ul_Vp(c.ret_D_Ul_Vp()));
}
...
test "D_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Us(c.ret_D_Us()));
}
...
test "D_Us_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Us_C(c.ret_D_Us_C()));
}
...
test "D_Us_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Us_D(c.ret_D_Us_D()));
}
...
test "D_Us_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Us_F(c.ret_D_Us_F()));
}
...
test "D_Us_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Us_I(c.ret_D_Us_I()));
}
...
test "D_Us_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Us_Ip(c.ret_D_Us_Ip()));
}
...
test "D_Us_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Us_L(c.ret_D_Us_L()));
}
...
test "D_Us_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Us_S(c.ret_D_Us_S()));
}
...
test "D_Us_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Us_Uc(c.ret_D_Us_Uc()));
}
...
test "D_Us_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Us_Ui(c.ret_D_Us_Ui()));
}
...
test "D_Us_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Us_Ul(c.ret_D_Us_Ul()));
}
...
test "D_Us_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Us_Us(c.ret_D_Us_Us()));
}
...
test "D_Us_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Us_Vp(c.ret_D_Us_Vp()));
}
...
test "D_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Vp(c.ret_D_Vp()));
}
...
test "D_Vp_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Vp_C(c.ret_D_Vp_C()));
}
...
test "D_Vp_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Vp_D(c.ret_D_Vp_D()));
}
...
test "D_Vp_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Vp_F(c.ret_D_Vp_F()));
}
...
test "D_Vp_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Vp_I(c.ret_D_Vp_I()));
}
...
test "D_Vp_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Vp_Ip(c.ret_D_Vp_Ip()));
}
...
test "D_Vp_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Vp_L(c.ret_D_Vp_L()));
}
...
test "D_Vp_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Vp_S(c.ret_D_Vp_S()));
}
...
test "D_Vp_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Vp_Uc(c.ret_D_Vp_Uc()));
}
...
test "D_Vp_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Vp_Ui(c.ret_D_Vp_Ui()));
}
...
test "D_Vp_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Vp_Ul(c.ret_D_Vp_Ul()));
}
...
test "D_Vp_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Vp_Us(c.ret_D_Vp_Us()));
}
...
test "D_Vp_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_D_Vp_Vp(c.ret_D_Vp_Vp()));
}
...
test "F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_F(c.ret_F()));
}
