...
test "Ip_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_C(c.ret_Ip_C()));
}
...
test "Ip_C_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_C_C(c.ret_Ip_C_C()));
}
...
test "Ip_C_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_C_D(c.ret_Ip_C_D()));
}
...
test "Ip_C_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_C_F(c.ret_Ip_C_F()));
}
...
test "Ip_C_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_C_I(c.ret_Ip_C_I()));
}
...
test "Ip_C_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_C_Ip(c.ret_Ip_C_Ip()));
}
...
test "Ip_C_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_C_L(c.ret_Ip_C_L()));
}
...
test "Ip_C_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_C_S(c.ret_Ip_C_S()));
}
...
test "Ip_C_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_C_Uc(c.ret_Ip_C_Uc()));
}
...
test "Ip_C_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_C_Ui(c.ret_Ip_C_Ui()));
}
...
test "Ip_C_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_C_Ul(c.ret_Ip_C_Ul()));
}
...
test "Ip_C_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_C_Us(c.ret_Ip_C_Us()));
}
...
test "Ip_C_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_C_Vp(c.ret_Ip_C_Vp()));
}
...
test "Ip_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_D(c.ret_Ip_D()));
}
...
test "Ip_D_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_D_C(c.ret_Ip_D_C()));
}
...
test "Ip_D_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_D_D(c.ret_Ip_D_D()));
}
...
test "Ip_D_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_D_F(c.ret_Ip_D_F()));
}
...
test "Ip_D_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_D_I(c.ret_Ip_D_I()));
}
...
test "Ip_D_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_D_Ip(c.ret_Ip_D_Ip()));
}
...
test "Ip_D_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_D_L(c.ret_Ip_D_L()));
}
...
test "Ip_D_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_D_S(c.ret_Ip_D_S()));
}
...
test "Ip_D_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_D_Uc(c.ret_Ip_D_Uc()));
}
...
test "Ip_D_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_D_Ui(c.ret_Ip_D_Ui()));
}
...
test "Ip_D_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_D_Ul(c.ret_Ip_D_Ul()));
}
...
test "Ip_D_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_D_Us(c.ret_Ip_D_Us()));
}
...
test "Ip_D_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_D_Vp(c.ret_Ip_D_Vp()));
}
...
test "Ip_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_F(c.ret_Ip_F()));
}
...
test "Ip_F_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_F_C(c.ret_Ip_F_C()));
}
...
test "Ip_F_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_F_D(c.ret_Ip_F_D()));
}
...
test "Ip_F_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_F_F(c.ret_Ip_F_F()));
}
...
test "Ip_F_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_F_I(c.ret_Ip_F_I()));
}
...
test "Ip_F_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_F_Ip(c.ret_Ip_F_Ip()));
}
...
test "Ip_F_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_F_L(c.ret_Ip_F_L()));
}
...
test "Ip_F_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_F_S(c.ret_Ip_F_S()));
}
...
test "Ip_F_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_F_Uc(c.ret_Ip_F_Uc()));
}
...
test "Ip_F_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_F_Ui(c.ret_Ip_F_Ui()));
}
...
test "Ip_F_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_F_Ul(c.ret_Ip_F_Ul()));
}
...
test "Ip_F_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_F_Us(c.ret_Ip_F_Us()));
}
...
test "Ip_F_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_F_Vp(c.ret_Ip_F_Vp()));
}
...
test "Ip_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_I(c.ret_Ip_I()));
}
...
test "Ip_I_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_I_C(c.ret_Ip_I_C()));
}
...
test "Ip_I_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_I_D(c.ret_Ip_I_D()));
}
...
test "Ip_I_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_I_F(c.ret_Ip_I_F()));
}
...
test "Ip_I_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_I_I(c.ret_Ip_I_I()));
}
...
test "Ip_I_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_I_Ip(c.ret_Ip_I_Ip()));
}
...
test "Ip_I_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_I_L(c.ret_Ip_I_L()));
}
...
test "Ip_I_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_I_S(c.ret_Ip_I_S()));
}
...
test "Ip_I_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_I_Uc(c.ret_Ip_I_Uc()));
}
...
test "Ip_I_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_I_Ui(c.ret_Ip_I_Ui()));
}
...
test "Ip_I_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_I_Ul(c.ret_Ip_I_Ul()));
}
...
test "Ip_I_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_I_Us(c.ret_Ip_I_Us()));
}
...
test "Ip_I_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_I_Vp(c.ret_Ip_I_Vp()));
}
...
test "Ip_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Ip(c.ret_Ip_Ip()));
}
...
test "Ip_Ip_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Ip_C(c.ret_Ip_Ip_C()));
}
...
test "Ip_Ip_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Ip_D(c.ret_Ip_Ip_D()));
}
...
test "Ip_Ip_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Ip_F(c.ret_Ip_Ip_F()));
}
...
test "Ip_Ip_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Ip_I(c.ret_Ip_Ip_I()));
}
...
test "Ip_Ip_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Ip_Ip(c.ret_Ip_Ip_Ip()));
}
...
test "Ip_Ip_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Ip_L(c.ret_Ip_Ip_L()));
}
...
test "Ip_Ip_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Ip_S(c.ret_Ip_Ip_S()));
}
...
test "Ip_Ip_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Ip_Uc(c.ret_Ip_Ip_Uc()));
}
...
test "Ip_Ip_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Ip_Ui(c.ret_Ip_Ip_Ui()));
}
...
test "Ip_Ip_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Ip_Ul(c.ret_Ip_Ip_Ul()));
}
...
test "Ip_Ip_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Ip_Us(c.ret_Ip_Ip_Us()));
}
...
test "Ip_Ip_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Ip_Vp(c.ret_Ip_Ip_Vp()));
}
...
test "Ip_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_L(c.ret_Ip_L()));
}
...
test "Ip_L_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_L_C(c.ret_Ip_L_C()));
}
...
test "Ip_L_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_L_D(c.ret_Ip_L_D()));
}
...
test "Ip_L_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_L_F(c.ret_Ip_L_F()));
}
...
test "Ip_L_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_L_I(c.ret_Ip_L_I()));
}
...
test "Ip_L_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_L_Ip(c.ret_Ip_L_Ip()));
}
...
test "Ip_L_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_L_L(c.ret_Ip_L_L()));
}
...
test "Ip_L_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_L_S(c.ret_Ip_L_S()));
}
...
test "Ip_L_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_L_Uc(c.ret_Ip_L_Uc()));
}
...
test "Ip_L_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_L_Ui(c.ret_Ip_L_Ui()));
}
...
test "Ip_L_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_L_Ul(c.ret_Ip_L_Ul()));
}
...
test "Ip_L_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_L_Us(c.ret_Ip_L_Us()));
}
...
test "Ip_L_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_L_Vp(c.ret_Ip_L_Vp()));
}
...
test "Ip_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_S(c.ret_Ip_S()));
}
...
test "Ip_S_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_S_C(c.ret_Ip_S_C()));
}
...
test "Ip_S_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_S_D(c.ret_Ip_S_D()));
}
...
test "Ip_S_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_S_F(c.ret_Ip_S_F()));
}
...
test "Ip_S_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_S_I(c.ret_Ip_S_I()));
}
...
test "Ip_S_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_S_Ip(c.ret_Ip_S_Ip()));
}
...
test "Ip_S_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_S_L(c.ret_Ip_S_L()));
}
...
test "Ip_S_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_S_S(c.ret_Ip_S_S()));
}
...
test "Ip_S_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_S_Uc(c.ret_Ip_S_Uc()));
}
...
test "Ip_S_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_S_Ui(c.ret_Ip_S_Ui()));
}
...
test "Ip_S_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_S_Ul(c.ret_Ip_S_Ul()));
}
...
test "Ip_S_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_S_Us(c.ret_Ip_S_Us()));
}
...
test "Ip_S_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_S_Vp(c.ret_Ip_S_Vp()));
}
...
test "Ip_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Uc(c.ret_Ip_Uc()));
}
...
test "Ip_Uc_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Uc_C(c.ret_Ip_Uc_C()));
}
...
test "Ip_Uc_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Uc_D(c.ret_Ip_Uc_D()));
}
...
test "Ip_Uc_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Uc_F(c.ret_Ip_Uc_F()));
}
...
test "Ip_Uc_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Uc_I(c.ret_Ip_Uc_I()));
}
...
test "Ip_Uc_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Uc_Ip(c.ret_Ip_Uc_Ip()));
}
...
test "Ip_Uc_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Uc_L(c.ret_Ip_Uc_L()));
}
...
test "Ip_Uc_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Uc_S(c.ret_Ip_Uc_S()));
}
...
test "Ip_Uc_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Uc_Uc(c.ret_Ip_Uc_Uc()));
}
...
test "Ip_Uc_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Uc_Ui(c.ret_Ip_Uc_Ui()));
}
...
test "Ip_Uc_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Uc_Ul(c.ret_Ip_Uc_Ul()));
}
...
test "Ip_Uc_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Uc_Us(c.ret_Ip_Uc_Us()));
}
...
test "Ip_Uc_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Uc_Vp(c.ret_Ip_Uc_Vp()));
}
...
test "Ip_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Ui(c.ret_Ip_Ui()));
}
...
test "Ip_Ui_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Ui_C(c.ret_Ip_Ui_C()));
}
...
test "Ip_Ui_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Ui_D(c.ret_Ip_Ui_D()));
}
...
test "Ip_Ui_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Ui_F(c.ret_Ip_Ui_F()));
}
...
test "Ip_Ui_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Ui_I(c.ret_Ip_Ui_I()));
}
...
test "Ip_Ui_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Ui_Ip(c.ret_Ip_Ui_Ip()));
}
...
test "Ip_Ui_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Ui_L(c.ret_Ip_Ui_L()));
}
...
test "Ip_Ui_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Ui_S(c.ret_Ip_Ui_S()));
}
...
test "Ip_Ui_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Ui_Uc(c.ret_Ip_Ui_Uc()));
}
...
test "Ip_Ui_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Ui_Ui(c.ret_Ip_Ui_Ui()));
}
...
test "Ip_Ui_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Ui_Ul(c.ret_Ip_Ui_Ul()));
}
...
test "Ip_Ui_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Ui_Us(c.ret_Ip_Ui_Us()));
}
...
test "Ip_Ui_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Ui_Vp(c.ret_Ip_Ui_Vp()));
}
...
test "Ip_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Ul(c.ret_Ip_Ul()));
}
...
test "Ip_Ul_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Ul_C(c.ret_Ip_Ul_C()));
}
...
test "Ip_Ul_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Ul_D(c.ret_Ip_Ul_D()));
}
...
test "Ip_Ul_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Ul_F(c.ret_Ip_Ul_F()));
}
...
test "Ip_Ul_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Ul_I(c.ret_Ip_Ul_I()));
}
...
test "Ip_Ul_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Ul_Ip(c.ret_Ip_Ul_Ip()));
}
...
test "Ip_Ul_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Ul_L(c.ret_Ip_Ul_L()));
}
...
test "Ip_Ul_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Ul_S(c.ret_Ip_Ul_S()));
}
...
test "Ip_Ul_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Ul_Uc(c.ret_Ip_Ul_Uc()));
}
...
test "Ip_Ul_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Ul_Ui(c.ret_Ip_Ul_Ui()));
}
...
test "Ip_Ul_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Ul_Ul(c.ret_Ip_Ul_Ul()));
}
...
test "Ip_Ul_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Ul_Us(c.ret_Ip_Ul_Us()));
}
...
test "Ip_Ul_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Ul_Vp(c.ret_Ip_Ul_Vp()));
}
...
test "Ip_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Us(c.ret_Ip_Us()));
}
...
test "Ip_Us_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Us_C(c.ret_Ip_Us_C()));
}
...
test "Ip_Us_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Us_D(c.ret_Ip_Us_D()));
}
...
test "Ip_Us_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Us_F(c.ret_Ip_Us_F()));
}
...
test "Ip_Us_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Us_I(c.ret_Ip_Us_I()));
}
...
test "Ip_Us_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Us_Ip(c.ret_Ip_Us_Ip()));
}
...
test "Ip_Us_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Us_L(c.ret_Ip_Us_L()));
}
...
test "Ip_Us_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Us_S(c.ret_Ip_Us_S()));
}
...
test "Ip_Us_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Us_Uc(c.ret_Ip_Us_Uc()));
}
...
test "Ip_Us_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Us_Ui(c.ret_Ip_Us_Ui()));
}
...
test "Ip_Us_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Us_Ul(c.ret_Ip_Us_Ul()));
}
...
test "Ip_Us_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Us_Us(c.ret_Ip_Us_Us()));
}
...
test "Ip_Us_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Us_Vp(c.ret_Ip_Us_Vp()));
}
...
test "Ip_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Vp(c.ret_Ip_Vp()));
}
...
test "Ip_Vp_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Vp_C(c.ret_Ip_Vp_C()));
}
...
test "Ip_Vp_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Vp_D(c.ret_Ip_Vp_D()));
}
...
test "Ip_Vp_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Vp_F(c.ret_Ip_Vp_F()));
}
...
test "Ip_Vp_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Vp_I(c.ret_Ip_Vp_I()));
}
...
test "Ip_Vp_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Vp_Ip(c.ret_Ip_Vp_Ip()));
}
...
test "Ip_Vp_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Vp_L(c.ret_Ip_Vp_L()));
}
...
test "Ip_Vp_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Vp_S(c.ret_Ip_Vp_S()));
}
...
test "Ip_Vp_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Vp_Uc(c.ret_Ip_Vp_Uc()));
}
...
test "Ip_Vp_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Vp_Ui(c.ret_Ip_Vp_Ui()));
}
...
test "Ip_Vp_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Vp_Ul(c.ret_Ip_Vp_Ul()));
}
...
test "Ip_Vp_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Vp_Us(c.ret_Ip_Vp_Us()));
}
...
test "Ip_Vp_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Ip_Vp_Vp(c.ret_Ip_Vp_Vp()));
}
...
test "L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L(c.ret_L()));
}
...
test "L_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_C(c.ret_L_C()));
}
...
test "L_C_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_C_C(c.ret_L_C_C()));
}
...
test "L_C_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_C_D(c.ret_L_C_D()));
}
...
test "L_C_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_C_F(c.ret_L_C_F()));
}
...
test "L_C_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_C_I(c.ret_L_C_I()));
}
...
test "L_C_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_C_Ip(c.ret_L_C_Ip()));
}
...
test "L_C_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_C_L(c.ret_L_C_L()));
}
...
test "L_C_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_C_S(c.ret_L_C_S()));
}
...
test "L_C_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_C_Uc(c.ret_L_C_Uc()));
}
...
test "L_C_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_C_Ui(c.ret_L_C_Ui()));
}
...
test "L_C_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_C_Ul(c.ret_L_C_Ul()));
}
...
test "L_C_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_C_Us(c.ret_L_C_Us()));
}
...
test "L_C_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_C_Vp(c.ret_L_C_Vp()));
}
...
test "L_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_D(c.ret_L_D()));
}
...
test "L_D_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_D_C(c.ret_L_D_C()));
}
...
test "L_D_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_D_D(c.ret_L_D_D()));
}
...
test "L_D_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_D_F(c.ret_L_D_F()));
}
...
test "L_D_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_D_I(c.ret_L_D_I()));
}
...
test "L_D_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_D_Ip(c.ret_L_D_Ip()));
}
...
test "L_D_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_D_L(c.ret_L_D_L()));
}
...
test "L_D_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_D_S(c.ret_L_D_S()));
}
...
test "L_D_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_D_Uc(c.ret_L_D_Uc()));
}
...
test "L_D_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_D_Ui(c.ret_L_D_Ui()));
}
...
test "L_D_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_D_Ul(c.ret_L_D_Ul()));
}
...
test "L_D_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_D_Us(c.ret_L_D_Us()));
}
...
test "L_D_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_D_Vp(c.ret_L_D_Vp()));
}
...
test "L_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_F(c.ret_L_F()));
}
...
test "L_F_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_F_C(c.ret_L_F_C()));
}
...
test "L_F_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_F_D(c.ret_L_F_D()));
}
...
test "L_F_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_F_F(c.ret_L_F_F()));
}
...
test "L_F_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_F_I(c.ret_L_F_I()));
}
...
test "L_F_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_F_Ip(c.ret_L_F_Ip()));
}
...
test "L_F_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_F_L(c.ret_L_F_L()));
}
...
test "L_F_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_F_S(c.ret_L_F_S()));
}
...
test "L_F_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_F_Uc(c.ret_L_F_Uc()));
}
...
test "L_F_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_F_Ui(c.ret_L_F_Ui()));
}
...
test "L_F_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_F_Ul(c.ret_L_F_Ul()));
}
...
test "L_F_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_F_Us(c.ret_L_F_Us()));
}
...
test "L_F_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_F_Vp(c.ret_L_F_Vp()));
}
...
test "L_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_I(c.ret_L_I()));
}
...
test "L_I_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_I_C(c.ret_L_I_C()));
}
...
test "L_I_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_I_D(c.ret_L_I_D()));
}
...
test "L_I_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_I_F(c.ret_L_I_F()));
}
...
test "L_I_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_I_I(c.ret_L_I_I()));
}
...
test "L_I_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_I_Ip(c.ret_L_I_Ip()));
}
...
test "L_I_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_I_L(c.ret_L_I_L()));
}
...
test "L_I_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_I_S(c.ret_L_I_S()));
}
...
test "L_I_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_I_Uc(c.ret_L_I_Uc()));
}
...
test "L_I_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_I_Ui(c.ret_L_I_Ui()));
}
...
test "L_I_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_I_Ul(c.ret_L_I_Ul()));
}
...
test "L_I_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_I_Us(c.ret_L_I_Us()));
}
...
test "L_I_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_I_Vp(c.ret_L_I_Vp()));
}
...
test "L_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Ip(c.ret_L_Ip()));
}
...
test "L_Ip_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Ip_C(c.ret_L_Ip_C()));
}
...
test "L_Ip_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Ip_D(c.ret_L_Ip_D()));
}
...
test "L_Ip_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Ip_F(c.ret_L_Ip_F()));
}
...
test "L_Ip_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Ip_I(c.ret_L_Ip_I()));
}
...
test "L_Ip_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Ip_Ip(c.ret_L_Ip_Ip()));
}
...
test "L_Ip_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Ip_L(c.ret_L_Ip_L()));
}
...
test "L_Ip_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Ip_S(c.ret_L_Ip_S()));
}
...
test "L_Ip_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Ip_Uc(c.ret_L_Ip_Uc()));
}
...
test "L_Ip_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Ip_Ui(c.ret_L_Ip_Ui()));
}
...
test "L_Ip_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Ip_Ul(c.ret_L_Ip_Ul()));
}
...
test "L_Ip_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Ip_Us(c.ret_L_Ip_Us()));
}
...
test "L_Ip_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Ip_Vp(c.ret_L_Ip_Vp()));
}
...
test "L_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_L(c.ret_L_L()));
}
...
test "L_L_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_L_C(c.ret_L_L_C()));
}
...
test "L_L_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_L_D(c.ret_L_L_D()));
}
...
test "L_L_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_L_F(c.ret_L_L_F()));
}
...
test "L_L_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_L_I(c.ret_L_L_I()));
}
...
test "L_L_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_L_Ip(c.ret_L_L_Ip()));
}
...
test "L_L_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_L_L(c.ret_L_L_L()));
}
...
test "L_L_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_L_S(c.ret_L_L_S()));
}
...
test "L_L_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_L_Uc(c.ret_L_L_Uc()));
}
...
test "L_L_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_L_Ui(c.ret_L_L_Ui()));
}
...
test "L_L_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_L_Ul(c.ret_L_L_Ul()));
}
...
test "L_L_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_L_Us(c.ret_L_L_Us()));
}
...
test "L_L_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_L_Vp(c.ret_L_L_Vp()));
}
...
test "L_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_S(c.ret_L_S()));
}
...
test "L_S_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_S_C(c.ret_L_S_C()));
}
...
test "L_S_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_S_D(c.ret_L_S_D()));
}
...
test "L_S_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_S_F(c.ret_L_S_F()));
}
...
test "L_S_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_S_I(c.ret_L_S_I()));
}
...
test "L_S_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_S_Ip(c.ret_L_S_Ip()));
}
...
test "L_S_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_S_L(c.ret_L_S_L()));
}
...
test "L_S_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_S_S(c.ret_L_S_S()));
}
...
test "L_S_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_S_Uc(c.ret_L_S_Uc()));
}
...
test "L_S_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_S_Ui(c.ret_L_S_Ui()));
}
...
test "L_S_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_S_Ul(c.ret_L_S_Ul()));
}
...
test "L_S_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_S_Us(c.ret_L_S_Us()));
}
...
test "L_S_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_S_Vp(c.ret_L_S_Vp()));
}
...
test "L_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Uc(c.ret_L_Uc()));
}
...
test "L_Uc_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Uc_C(c.ret_L_Uc_C()));
}
...
test "L_Uc_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Uc_D(c.ret_L_Uc_D()));
}
...
test "L_Uc_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Uc_F(c.ret_L_Uc_F()));
}
...
test "L_Uc_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Uc_I(c.ret_L_Uc_I()));
}
...
test "L_Uc_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Uc_Ip(c.ret_L_Uc_Ip()));
}
...
test "L_Uc_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Uc_L(c.ret_L_Uc_L()));
}
...
test "L_Uc_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Uc_S(c.ret_L_Uc_S()));
}
...
test "L_Uc_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Uc_Uc(c.ret_L_Uc_Uc()));
}
...
test "L_Uc_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Uc_Ui(c.ret_L_Uc_Ui()));
}
...
test "L_Uc_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Uc_Ul(c.ret_L_Uc_Ul()));
}
...
test "L_Uc_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Uc_Us(c.ret_L_Uc_Us()));
}
...
test "L_Uc_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Uc_Vp(c.ret_L_Uc_Vp()));
}
...
test "L_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Ui(c.ret_L_Ui()));
}
