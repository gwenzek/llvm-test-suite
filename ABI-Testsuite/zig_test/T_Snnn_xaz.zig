...
test "L_Ui_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Ui_C(c.ret_L_Ui_C()));
}
...
test "L_Ui_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Ui_D(c.ret_L_Ui_D()));
}
...
test "L_Ui_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Ui_F(c.ret_L_Ui_F()));
}
...
test "L_Ui_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Ui_I(c.ret_L_Ui_I()));
}
...
test "L_Ui_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Ui_Ip(c.ret_L_Ui_Ip()));
}
...
test "L_Ui_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Ui_L(c.ret_L_Ui_L()));
}
...
test "L_Ui_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Ui_S(c.ret_L_Ui_S()));
}
...
test "L_Ui_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Ui_Uc(c.ret_L_Ui_Uc()));
}
...
test "L_Ui_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Ui_Ui(c.ret_L_Ui_Ui()));
}
...
test "L_Ui_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Ui_Ul(c.ret_L_Ui_Ul()));
}
...
test "L_Ui_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Ui_Us(c.ret_L_Ui_Us()));
}
...
test "L_Ui_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Ui_Vp(c.ret_L_Ui_Vp()));
}
...
test "L_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Ul(c.ret_L_Ul()));
}
...
test "L_Ul_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Ul_C(c.ret_L_Ul_C()));
}
...
test "L_Ul_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Ul_D(c.ret_L_Ul_D()));
}
...
test "L_Ul_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Ul_F(c.ret_L_Ul_F()));
}
...
test "L_Ul_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Ul_I(c.ret_L_Ul_I()));
}
...
test "L_Ul_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Ul_Ip(c.ret_L_Ul_Ip()));
}
...
test "L_Ul_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Ul_L(c.ret_L_Ul_L()));
}
...
test "L_Ul_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Ul_S(c.ret_L_Ul_S()));
}
...
test "L_Ul_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Ul_Uc(c.ret_L_Ul_Uc()));
}
...
test "L_Ul_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Ul_Ui(c.ret_L_Ul_Ui()));
}
...
test "L_Ul_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Ul_Ul(c.ret_L_Ul_Ul()));
}
...
test "L_Ul_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Ul_Us(c.ret_L_Ul_Us()));
}
...
test "L_Ul_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Ul_Vp(c.ret_L_Ul_Vp()));
}
...
test "L_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Us(c.ret_L_Us()));
}
...
test "L_Us_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Us_C(c.ret_L_Us_C()));
}
...
test "L_Us_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Us_D(c.ret_L_Us_D()));
}
...
test "L_Us_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Us_F(c.ret_L_Us_F()));
}
...
test "L_Us_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Us_I(c.ret_L_Us_I()));
}
...
test "L_Us_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Us_Ip(c.ret_L_Us_Ip()));
}
...
test "L_Us_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Us_L(c.ret_L_Us_L()));
}
...
test "L_Us_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Us_S(c.ret_L_Us_S()));
}
...
test "L_Us_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Us_Uc(c.ret_L_Us_Uc()));
}
...
test "L_Us_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Us_Ui(c.ret_L_Us_Ui()));
}
...
test "L_Us_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Us_Ul(c.ret_L_Us_Ul()));
}
...
test "L_Us_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Us_Us(c.ret_L_Us_Us()));
}
...
test "L_Us_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Us_Vp(c.ret_L_Us_Vp()));
}
...
test "L_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Vp(c.ret_L_Vp()));
}
...
test "L_Vp_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Vp_C(c.ret_L_Vp_C()));
}
...
test "L_Vp_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Vp_D(c.ret_L_Vp_D()));
}
...
test "L_Vp_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Vp_F(c.ret_L_Vp_F()));
}
...
test "L_Vp_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Vp_I(c.ret_L_Vp_I()));
}
...
test "L_Vp_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Vp_Ip(c.ret_L_Vp_Ip()));
}
...
test "L_Vp_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Vp_L(c.ret_L_Vp_L()));
}
...
test "L_Vp_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Vp_S(c.ret_L_Vp_S()));
}
...
test "L_Vp_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Vp_Uc(c.ret_L_Vp_Uc()));
}
...
test "L_Vp_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Vp_Ui(c.ret_L_Vp_Ui()));
}
...
test "L_Vp_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Vp_Ul(c.ret_L_Vp_Ul()));
}
...
test "L_Vp_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Vp_Us(c.ret_L_Vp_Us()));
}
...
test "L_Vp_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_L_Vp_Vp(c.ret_L_Vp_Vp()));
}
...
test "S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S(c.ret_S()));
}
...
test "S_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_C(c.ret_S_C()));
}
...
test "S_C_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_C_C(c.ret_S_C_C()));
}
...
test "S_C_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_C_D(c.ret_S_C_D()));
}
...
test "S_C_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_C_F(c.ret_S_C_F()));
}
...
test "S_C_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_C_I(c.ret_S_C_I()));
}
...
test "S_C_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_C_Ip(c.ret_S_C_Ip()));
}
...
test "S_C_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_C_L(c.ret_S_C_L()));
}
...
test "S_C_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_C_S(c.ret_S_C_S()));
}
...
test "S_C_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_C_Uc(c.ret_S_C_Uc()));
}
...
test "S_C_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_C_Ui(c.ret_S_C_Ui()));
}
...
test "S_C_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_C_Ul(c.ret_S_C_Ul()));
}
...
test "S_C_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_C_Us(c.ret_S_C_Us()));
}
...
test "S_C_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_C_Vp(c.ret_S_C_Vp()));
}
...
test "S_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_D(c.ret_S_D()));
}
...
test "S_D_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_D_C(c.ret_S_D_C()));
}
...
test "S_D_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_D_D(c.ret_S_D_D()));
}
...
test "S_D_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_D_F(c.ret_S_D_F()));
}
...
test "S_D_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_D_I(c.ret_S_D_I()));
}
...
test "S_D_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_D_Ip(c.ret_S_D_Ip()));
}
...
test "S_D_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_D_L(c.ret_S_D_L()));
}
...
test "S_D_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_D_S(c.ret_S_D_S()));
}
...
test "S_D_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_D_Uc(c.ret_S_D_Uc()));
}
...
test "S_D_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_D_Ui(c.ret_S_D_Ui()));
}
...
test "S_D_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_D_Ul(c.ret_S_D_Ul()));
}
...
test "S_D_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_D_Us(c.ret_S_D_Us()));
}
...
test "S_D_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_D_Vp(c.ret_S_D_Vp()));
}
...
test "S_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_F(c.ret_S_F()));
}
...
test "S_F_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_F_C(c.ret_S_F_C()));
}
...
test "S_F_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_F_D(c.ret_S_F_D()));
}
...
test "S_F_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_F_F(c.ret_S_F_F()));
}
...
test "S_F_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_F_I(c.ret_S_F_I()));
}
...
test "S_F_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_F_Ip(c.ret_S_F_Ip()));
}
...
test "S_F_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_F_L(c.ret_S_F_L()));
}
...
test "S_F_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_F_S(c.ret_S_F_S()));
}
...
test "S_F_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_F_Uc(c.ret_S_F_Uc()));
}
...
test "S_F_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_F_Ui(c.ret_S_F_Ui()));
}
...
test "S_F_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_F_Ul(c.ret_S_F_Ul()));
}
...
test "S_F_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_F_Us(c.ret_S_F_Us()));
}
...
test "S_F_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_F_Vp(c.ret_S_F_Vp()));
}
...
test "S_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_I(c.ret_S_I()));
}
...
test "S_I_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_I_C(c.ret_S_I_C()));
}
...
test "S_I_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_I_D(c.ret_S_I_D()));
}
...
test "S_I_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_I_F(c.ret_S_I_F()));
}
...
test "S_I_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_I_I(c.ret_S_I_I()));
}
...
test "S_I_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_I_Ip(c.ret_S_I_Ip()));
}
...
test "S_I_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_I_L(c.ret_S_I_L()));
}
...
test "S_I_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_I_S(c.ret_S_I_S()));
}
...
test "S_I_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_I_Uc(c.ret_S_I_Uc()));
}
...
test "S_I_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_I_Ui(c.ret_S_I_Ui()));
}
...
test "S_I_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_I_Ul(c.ret_S_I_Ul()));
}
...
test "S_I_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_I_Us(c.ret_S_I_Us()));
}
...
test "S_I_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_I_Vp(c.ret_S_I_Vp()));
}
...
test "S_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Ip(c.ret_S_Ip()));
}
...
test "S_Ip_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Ip_C(c.ret_S_Ip_C()));
}
...
test "S_Ip_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Ip_D(c.ret_S_Ip_D()));
}
...
test "S_Ip_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Ip_F(c.ret_S_Ip_F()));
}
...
test "S_Ip_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Ip_I(c.ret_S_Ip_I()));
}
...
test "S_Ip_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Ip_Ip(c.ret_S_Ip_Ip()));
}
...
test "S_Ip_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Ip_L(c.ret_S_Ip_L()));
}
...
test "S_Ip_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Ip_S(c.ret_S_Ip_S()));
}
...
test "S_Ip_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Ip_Uc(c.ret_S_Ip_Uc()));
}
...
test "S_Ip_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Ip_Ui(c.ret_S_Ip_Ui()));
}
...
test "S_Ip_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Ip_Ul(c.ret_S_Ip_Ul()));
}
...
test "S_Ip_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Ip_Us(c.ret_S_Ip_Us()));
}
...
test "S_Ip_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Ip_Vp(c.ret_S_Ip_Vp()));
}
...
test "S_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_L(c.ret_S_L()));
}
...
test "S_L_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_L_C(c.ret_S_L_C()));
}
...
test "S_L_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_L_D(c.ret_S_L_D()));
}
...
test "S_L_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_L_F(c.ret_S_L_F()));
}
...
test "S_L_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_L_I(c.ret_S_L_I()));
}
...
test "S_L_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_L_Ip(c.ret_S_L_Ip()));
}
...
test "S_L_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_L_L(c.ret_S_L_L()));
}
...
test "S_L_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_L_S(c.ret_S_L_S()));
}
...
test "S_L_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_L_Uc(c.ret_S_L_Uc()));
}
...
test "S_L_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_L_Ui(c.ret_S_L_Ui()));
}
...
test "S_L_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_L_Ul(c.ret_S_L_Ul()));
}
...
test "S_L_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_L_Us(c.ret_S_L_Us()));
}
...
test "S_L_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_L_Vp(c.ret_S_L_Vp()));
}
...
test "S_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_S(c.ret_S_S()));
}
...
test "S_S_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_S_C(c.ret_S_S_C()));
}
...
test "S_S_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_S_D(c.ret_S_S_D()));
}
...
test "S_S_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_S_F(c.ret_S_S_F()));
}
...
test "S_S_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_S_I(c.ret_S_S_I()));
}
...
test "S_S_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_S_Ip(c.ret_S_S_Ip()));
}
...
test "S_S_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_S_L(c.ret_S_S_L()));
}
...
test "S_S_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_S_S(c.ret_S_S_S()));
}
...
test "S_S_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_S_Uc(c.ret_S_S_Uc()));
}
...
test "S_S_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_S_Ui(c.ret_S_S_Ui()));
}
...
test "S_S_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_S_Ul(c.ret_S_S_Ul()));
}
...
test "S_S_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_S_Us(c.ret_S_S_Us()));
}
...
test "S_S_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_S_Vp(c.ret_S_S_Vp()));
}
...
test "S_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Uc(c.ret_S_Uc()));
}
...
test "S_Uc_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Uc_C(c.ret_S_Uc_C()));
}
...
test "S_Uc_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Uc_D(c.ret_S_Uc_D()));
}
...
test "S_Uc_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Uc_F(c.ret_S_Uc_F()));
}
...
test "S_Uc_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Uc_I(c.ret_S_Uc_I()));
}
...
test "S_Uc_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Uc_Ip(c.ret_S_Uc_Ip()));
}
...
test "S_Uc_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Uc_L(c.ret_S_Uc_L()));
}
...
test "S_Uc_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Uc_S(c.ret_S_Uc_S()));
}
...
test "S_Uc_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Uc_Uc(c.ret_S_Uc_Uc()));
}
...
test "S_Uc_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Uc_Ui(c.ret_S_Uc_Ui()));
}
...
test "S_Uc_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Uc_Ul(c.ret_S_Uc_Ul()));
}
...
test "S_Uc_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Uc_Us(c.ret_S_Uc_Us()));
}
...
test "S_Uc_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Uc_Vp(c.ret_S_Uc_Vp()));
}
...
test "S_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Ui(c.ret_S_Ui()));
}
...
test "S_Ui_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Ui_C(c.ret_S_Ui_C()));
}
...
test "S_Ui_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Ui_D(c.ret_S_Ui_D()));
}
...
test "S_Ui_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Ui_F(c.ret_S_Ui_F()));
}
...
test "S_Ui_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Ui_I(c.ret_S_Ui_I()));
}
...
test "S_Ui_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Ui_Ip(c.ret_S_Ui_Ip()));
}
...
test "S_Ui_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Ui_L(c.ret_S_Ui_L()));
}
...
test "S_Ui_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Ui_S(c.ret_S_Ui_S()));
}
...
test "S_Ui_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Ui_Uc(c.ret_S_Ui_Uc()));
}
...
test "S_Ui_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Ui_Ui(c.ret_S_Ui_Ui()));
}
...
test "S_Ui_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Ui_Ul(c.ret_S_Ui_Ul()));
}
...
test "S_Ui_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Ui_Us(c.ret_S_Ui_Us()));
}
...
test "S_Ui_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Ui_Vp(c.ret_S_Ui_Vp()));
}
...
test "S_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Ul(c.ret_S_Ul()));
}
...
test "S_Ul_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Ul_C(c.ret_S_Ul_C()));
}
...
test "S_Ul_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Ul_D(c.ret_S_Ul_D()));
}
...
test "S_Ul_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Ul_F(c.ret_S_Ul_F()));
}
...
test "S_Ul_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Ul_I(c.ret_S_Ul_I()));
}
...
test "S_Ul_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Ul_Ip(c.ret_S_Ul_Ip()));
}
...
test "S_Ul_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Ul_L(c.ret_S_Ul_L()));
}
...
test "S_Ul_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Ul_S(c.ret_S_Ul_S()));
}
...
test "S_Ul_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Ul_Uc(c.ret_S_Ul_Uc()));
}
...
test "S_Ul_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Ul_Ui(c.ret_S_Ul_Ui()));
}
...
test "S_Ul_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Ul_Ul(c.ret_S_Ul_Ul()));
}
...
test "S_Ul_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Ul_Us(c.ret_S_Ul_Us()));
}
...
test "S_Ul_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Ul_Vp(c.ret_S_Ul_Vp()));
}
...
test "S_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Us(c.ret_S_Us()));
}
...
test "S_Us_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Us_C(c.ret_S_Us_C()));
}
...
test "S_Us_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Us_D(c.ret_S_Us_D()));
}
...
test "S_Us_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Us_F(c.ret_S_Us_F()));
}
...
test "S_Us_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Us_I(c.ret_S_Us_I()));
}
...
test "S_Us_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Us_Ip(c.ret_S_Us_Ip()));
}
...
test "S_Us_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Us_L(c.ret_S_Us_L()));
}
...
test "S_Us_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Us_S(c.ret_S_Us_S()));
}
...
test "S_Us_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Us_Uc(c.ret_S_Us_Uc()));
}
...
test "S_Us_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Us_Ui(c.ret_S_Us_Ui()));
}
...
test "S_Us_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Us_Ul(c.ret_S_Us_Ul()));
}
...
test "S_Us_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Us_Us(c.ret_S_Us_Us()));
}
...
test "S_Us_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Us_Vp(c.ret_S_Us_Vp()));
}
...
test "S_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Vp(c.ret_S_Vp()));
}
...
test "S_Vp_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Vp_C(c.ret_S_Vp_C()));
}
...
test "S_Vp_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Vp_D(c.ret_S_Vp_D()));
}
...
test "S_Vp_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Vp_F(c.ret_S_Vp_F()));
}
...
test "S_Vp_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Vp_I(c.ret_S_Vp_I()));
}
...
test "S_Vp_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Vp_Ip(c.ret_S_Vp_Ip()));
}
...
test "S_Vp_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Vp_L(c.ret_S_Vp_L()));
}
...
test "S_Vp_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Vp_S(c.ret_S_Vp_S()));
}
...
test "S_Vp_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Vp_Uc(c.ret_S_Vp_Uc()));
}
...
test "S_Vp_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Vp_Ui(c.ret_S_Vp_Ui()));
}
...
test "S_Vp_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Vp_Ul(c.ret_S_Vp_Ul()));
}
...
test "S_Vp_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Vp_Us(c.ret_S_Vp_Us()));
}
...
test "S_Vp_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_S_Vp_Vp(c.ret_S_Vp_Vp()));
}
...
test "Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Uc(c.ret_Uc()));
}
...
test "Uc_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Uc_C(c.ret_Uc_C()));
}
...
test "Uc_C_C: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Uc_C_C(c.ret_Uc_C_C()));
}
...
test "Uc_C_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Uc_C_D(c.ret_Uc_C_D()));
}
...
test "Uc_C_F: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Uc_C_F(c.ret_Uc_C_F()));
}
...
test "Uc_C_I: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Uc_C_I(c.ret_Uc_C_I()));
}
...
test "Uc_C_Ip: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Uc_C_Ip(c.ret_Uc_C_Ip()));
}
...
test "Uc_C_L: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Uc_C_L(c.ret_Uc_C_L()));
}
...
test "Uc_C_S: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Uc_C_S(c.ret_Uc_C_S()));
}
...
test "Uc_C_Uc: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Uc_C_Uc(c.ret_Uc_C_Uc()));
}
...
test "Uc_C_Ui: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Uc_C_Ui(c.ret_Uc_C_Ui()));
}
...
test "Uc_C_Ul: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Uc_C_Ul(c.ret_Uc_C_Ul()));
}
...
test "Uc_C_Us: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Uc_C_Us(c.ret_Uc_C_Us()));
}
...
test "Uc_C_Vp: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Uc_C_Vp(c.ret_Uc_C_Vp()));
}
...
test "Uc_D: C returns to Zig" {
    if (comptime builtin.cpu.arch.isPPC()) return error.SkipZigTest;
    if (builtin.cpu.arch == .x86) return error.SkipZigTest;
    try testing.expectOk(zig_assert_Uc_D(c.ret_Uc_D()));
}
